...
/**
 * Generate metadata for the Arabic contact page using WordPress SEO data
 */
export async function generateMetadata(): Promise<Metadata> {
  ...
  const seoData = await getCachedSeoData('ar', contactPageId);
  
  // Default values
  const defaultTitle = 'اتصل بنا - عالم القهوة';
  const defaultDescription = 'تواصل مع عالم القهوة. اتصل بنا للاستفسارات أو التعليقات أو لمعرفة المزيد عن أخبار القهوة وقصصنا.';
  ...
  const keywords = seoData?.metaKeywords || 'اتصل بنا, عالم القهوة, قهوة, استفسار, تعليقات';
  const canonical = normalizeUrl(seoData?.canonical) || `${siteUrl}/ar/contact`;
