...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!backendArticles || backendArticles.length === 0) {
    ...
    return {
      ...
      description: `تصفح المقالات في قسم ${categoryName}`,
    };
  }
  ...
  const description = seoData?.metaDesc || `تصفح المقالات في قسم ${categoryName} على Qahwa World`;
  ...
  return {
    title,
    description,
    keywords: seoData?.metaKeywords || `${categoryName}, قهوة, قهوة عربية, مقالات`,
    alternates: {
      canonical,
    },
    openGraph: {
...
export default async function CategoryRoute({ params, locale = 'ar' }: Props) {
  const { category } = await params;
  ...
  const collectionSchema = articles.length > 0 
    ? generateCollectionPageSchema(
        categoryName,
        firstArticleSeo?.metaDesc || `تصفح المقالات في قسم ${categoryName}`,
        categoryUrl,
        articles,
        locale
      )
