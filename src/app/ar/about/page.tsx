...
/**
 * Generate metadata for the Arabic about page using WordPress SEO data
 */
export async function generateMetadata(): Promise<Metadata> {
  ...
  const seoData = await getCachedSeoData('ar', aboutPageId);
  
  // Default values
  const defaultTitle = 'من نحن - عالم القهوة';
  const defaultDescription = 'تعرف على عالم القهوة، مهمتنا، رؤيتنا، وقيمنا. اكتشف التزامنا بتقديم أحدث أخبار القهوة والقصص من جميع أنحاء العالم.';
  ...
  const keywords = seoData?.metaKeywords || 'من نحن، قهوة، عالم القهوة، المهمة، الرؤية، القيم';
  const canonical = normalizeUrl(seoData?.canonical) || `${siteUrl}/ar/about`;
