...
/**
 * Generate metadata for the Arabic home page using WordPress SEO data
 */
export async function generateMetadata(): Promise<Metadata> {
  ...
  // Default values
  const defaultTitle = 'عالم القهوة - ثقافة وأخبار القهوة';
  const defaultDescription = 'اكتشف أحدث أخبار القهوة وتقنيات التحضير والقصص من جميع أنحاء العالم.';
  ...
  const keywords = seoData?.metaKeywords || 'قهوة، قهوة عربية، أخبار القهوة، تحضير القهوة، ثقافة القهوة';
  const canonical = normalizeUrl(seoData?.canonical) || `${siteUrl}/ar`;
