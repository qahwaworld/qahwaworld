...
function generateNewsSitemapXml(
  ...
  const urlEntries = posts
    .map((post) => {
      ...
      // Build URL with language prefix (except for English)
      const articlePath = `/${categorySlug}/${post.slug}`;
      const fullPath = normalizedLang === 'en' 
        ? articlePath 
        : getLocalizedPath(articlePath, normalizedLang);
...
export async function GET() {
  try {
    // Fetch latest 50 posts for English (default)
    const posts = await getLatestPostsForNewsSitemap('en', 50);

    // Generate XML
    const newsSitemapXml = generateNewsSitemapXml(posts, 'en');
