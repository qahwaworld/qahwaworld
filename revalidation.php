 ...
 * 3. Update NEXTJS_FRONTEND_URL (in wp-functions.php) and NEXTJS_SECRET below
...
// ===============================
// Note: NEXTJS_FRONTEND_URL is defined in wp-functions.php
// If not defined there, we'll use a fallback here
if (!defined('NEXTJS_FRONTEND_URL')) {
    ...
    //    Then update NEXTJS_FRONTEND_URL in wp-functions.php: 'https://your-nextjs-domain.com'
    // 
    // OPTION 2: Use ngrok to expose localhost (for development/testing)
    //    1. Install ngrok: brew install ngrok (or download from ngrok.com)
    //    2. Run: ngrok http 3000
    //    3. Copy the HTTPS URL (e.g., https://abc123.ngrok.io)
    //    4. Update NEXTJS_FRONTEND_URL in wp-functions.php: 'https://abc123.ngrok.io'
    // 
...
function send_revalidation_webhook($post_id, $post, $action) {
    ...
    if (!defined('NEXTJS_FRONTEND_URL')) {
        error_log('⚠️ NEXTJS_FRONTEND_URL not defined in wp-functions.php');
        return;
    }
    ...
    $base_url = rtrim(NEXTJS_FRONTEND_URL, '/');
            ...
            foreach ($fallback_urls as $fallback_url) {
                ...
                if (!is_wp_error($fallback_response)) {
                    ...
                    if ($response_code === 200) {
                        ...
                        error_log('💡 Consider updating NEXTJS_FRONTEND_URL in wp-functions.php to: ' . $fallback_url);
...
function test_nextjs_revalidation() {
    if (isset($_GET['test_nextjs_revalidation']) && current_user_can('manage_options')) {
        ...
        echo '<p><strong>NEXTJS_FRONTEND_URL:</strong> ' . (defined('NEXTJS_FRONTEND_URL') ? NEXTJS_FRONTEND_URL : '❌ Not defined') . '</p>';
